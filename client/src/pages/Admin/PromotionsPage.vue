      ...
      <div class="modal-content large">
        ...
        <form @submit.prevent="savePromotion" class="promo-form">
          ...
          <div class="form-section">
            ...
            <div v-if="form.type === 'buy_x_get_y'" class="form-row">
              <div class="form-group">
  ...
  methods: {
    ...
    async savePromotion() {
      ...
      try {
        if (this.editingPromo) {
          await axios.put(`/promotions/${this.editingPromo.id}`, this.form)
          this.showAlert('Promotion updated successfully', 'success')
        } else {
          await axios.post('/promotions', this.form)
    ...
    formatDateRange(promo) {
      if (promo.start_date && promo.end_date) {
        return `${this.formatDate(promo.start_date)} - ${this.formatDate(promo.end_date)}`
      } else if (promo.start_date) {
        return `From ${this.formatDate(promo.start_date)}`
      } else if (promo.end_date) {
        return `Until ${this.formatDate(promo.end_date)}`
      }
