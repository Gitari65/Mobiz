...
const getProfilePictureUrl = (path) => {
  ...
  // Otherwise, construct URL to Laravel storage
  return `${axios.defaults.baseURL}/storage/${path}`
}

const handleImageError = (e) => {
  e.target.style.display = 'none'
  showAlert('error', 'Failed to load profile picture')
}
...
const loadProfile = async () => {
  try {
    ...
    showAlert('error', error.response?.data?.message || 'Failed to load profile')
  } finally {
...
const removeProfilePicture = async () => {
  ...
  try {
    ...
    showAlert('error', error.response?.data?.message || 'Failed to remove profile picture')
  } finally {
    isLoading.value = false
  }
}
...
const changePassword = async () => {
  ...
  try {
    ...
    showAlert('success', 'Password changed successfully! Please use your new password for future logins.', 5000)
  } catch (error) {
...
const requestPasswordReset = async () => {
  ...
  try {
    ...
    showAlert('success', `A new temporary password has been sent to ${profileData.email}. Please check your inbox and follow the instructions to login.`, 8000)
  } catch (error) {
